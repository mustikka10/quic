			   ...
			   const void *rx_secret, const void *tx_secret, size_t secretlen)
{
	...
	if (tx_secret) {
		...
		/* Set EARLY TX secret via setsockopt, which will send any buffered early
		 * data in the kernel. So pass more_data = 1.
		 */
		quic_smsg_append_list(ctx, smsg, level == GNUTLS_ENCRYPTION_LEVEL_EARLY);
	}
	if (rx_secret) {
		secret.send = 0;
		 ...
		 * send more data (NCI frames) in the kernel. So pass more_data = 1 for
		 * this case.
		 */
